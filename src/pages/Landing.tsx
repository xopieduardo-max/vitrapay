...
import ctaVitrapay from "@/assets/cta-vitrapay-team.webp.asset.json";
import bentoAprovacao from "@/assets/bento-aprovacao.webp.asset.json";
...
export default function Landing() {
  ...
  return (
    <div className="min-h-screen bg-background">
        ...
        <div className="container relative py-2 md:py-4">
          <div className="relative isolate">
            ...
            >
              ...
              {/* Todas atrás do brasão (z-0), contidas dentro da coluna direita para não invadir o texto */}
              {/* Topo esquerda — blur */}
              <StaticSaleNotification
                method="Pix"
                value="127,00"
                blurred
                className="hidden md:flex absolute z-0 top-[6%] left-[2%] w-[240px] lg:w-[260px] opacity-70"
              ...
              <StaticSaleNotification
                method="Boleto"
                value="47,00"
                className="hidden md:flex absolute z-0 top-[10%] right-[2%] w-[250px] lg:w-[270px] opacity-90"
                style={{ animation: "floatY 6s ease-in-out infinite 0.5s" }}
              />

              {/* Inferior esquerda — nítida */}
              <StaticSaleNotification
                method="Pix"
                value="90,00"
                className="hidden md:flex absolute z-0 bottom-[8%] left-[1%] w-[250px] lg:w-[270px] opacity-90"
                style={{ animation: "floatY 5.5s ease-in-out infinite 1s" }}
              />

              {/* Inferior direita — blur */}
              <StaticSaleNotification
                method="Cartão"
                value="349,90"
                blurred
                className="hidden md:flex absolute z-0 bottom-[6%] right-[2%] w-[240px] lg:w-[260px] opacity-70"
      ...
      <section className="relative bg-[#080808] border-y border-white/[0.06] overflow-hidden">
        ...
        <div className="container py-8 md:py-12">
          <motion.div
            ...
            className="flex items-center justify-center gap-2.5 sm:gap-3 md:gap-4 lg:gap-6"
          >
            {/* Bloco esquerdo: "Trabalhar / com a" empilhado, alinhado à direita */}
            <div className="text-right leading-[0.95] text-white font-light text-sm sm:text-xl md:text-2xl lg:text-[2.6rem]">
              <div>Trabalhar</div>
              <div>com a</div>
            </div>
            ...
            <div className="flex items-center gap-1 sm:gap-1.5 md:gap-2 lg:gap-2.5 shrink-0">
              <img
                src={logoIcon}
                alt="VitraPay"
                className="h-7 sm:h-9 md:h-12 lg:h-[68px] object-contain"
                loading="lazy"
              />
              <span className="text-white font-bold leading-none text-lg sm:text-2xl md:text-[2rem] lg:text-[4.7rem] tracking-tight">
                VitraPay
              </span>
            </div>

            {/* Bloco direito: "é muita / vantagem!" empilhado, alinhado à esquerda, em amarelo */}
            <div className="text-left leading-[0.95] text-primary text-sm sm:text-xl md:text-2xl lg:text-[2.6rem]">
        ...
        {/* Marquee logo abaixo */}
        <div className="relative pb-6 space-y-4">
      ...
      {/* ─── FAQ Section ─── */}
      <section id="faq" className="relative overflow-hidden bg-[#080808] py-24 md:py-32">
        {/* Subtle radial glow behind title */}
        <div className="absolute left-0 top-1/2 -translate-y-1/2 w-[600px] h-[600px] rounded-full bg-primary/[0.04] blur-[120px] pointer-events-none" />

        <div className="container relative grid lg:grid-cols-2 gap-12 lg:gap-16 items-start">
          {/* Left: Title + CTA */}
          <motion.div {...fadeUp} transition={{ duration: 0.6 }} className="lg:sticky lg:top-32 space-y-10">
            <h2 className="text-5xl md:text-6xl lg:text-7xl font-bold tracking-tight leading-[0.95] text-white">
              Dúvidas
              <br />
            ...
            <p className="text-base md:text-lg text-white/60 leading-relaxed max-w-md">
              Caso não encontre sua dúvida aqui, fale com o nosso time de atendimento ao cliente.
            </p>
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-3 rounded-2xl border border-primary/40 bg-primary/[0.06] px-7 py-4 text-primary font-semibold hover:bg-primary/[0.12] hover:border-primary/60 transition-all duration-200 hover:-translate-y-0.5"
            >
            ...
            <Accordion type="single" collapsible className="space-y-1">
              {faqItems.map((item, i) =>
                <AccordionItem
                  key={i}
                  value={`faq-${i}`}
                  className="border-0 border-b border-white/10 last:border-b-0"
                >
                  <AccordionTrigger className="hover:no-underline text-left py-6 [&[data-state=open]>svg]:text-primary [&>svg]:text-white/40 [&>svg]:transition-colors">
                  ...
                  <AccordionContent className="pl-12 pr-4 pb-6 text-sm md:text-base text-white/60 leading-relaxed">
                    {item.a === "HELP_LINK" ? (
