...
export default function Checkout() {
  ...
  useEffect(() => {
    ...
    const loadCheckout = async () => {
      ...
      if (ref) {
        supabase.rpc("increment_affiliate_clicks" as any, { affiliate_id: ref }).then(() => {}).catch(() => {});
      }
  ...
  return (
    <div className={`min-h-screen ${product.checkout_theme === 'light' ? 'checkout-light' : 'checkout-dark'}`} style={{ background: "var(--ck-bg)", color: "var(--ck-fg)" }}>
      ...
      {product.checkout_headline && (
        <div className="py-2.5 text-center text-sm font-bold tracking-wide" style={{ background: "hsl(48, 96%, 53%)", color: "hsl(0,0%,10%)" }}>
          {product.checkout_headline}
        </div>
      )}
